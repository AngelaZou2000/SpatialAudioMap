...
#define sine_table_size 256 // sine lookup table for DDS
...
// phase increment to set the frequency DDS_increment = Fout*two32/Fs; A above middle C: Fout = 440.0
...
void __ISR(_TIMER_3_VECTOR, ipl1) Timer3Handler(void)
{
    mT3ClearIntFlag();
    timer3_counter++;
    // set one of the note time to 0 to enable the audio
    if (left == 1) {
...
static PT_THREAD (protothread_timer(struct pt *pt))
{
    PT_BEGIN(pt);
      ...
      while(1) {
        ...
        adc5_y = ReadADC10(0);   
        adc11_x = ReadADC10(1);   
        tft_fillCircle(Accum2int(xpos), Accum2int(ypos), 4, ILI9340_BLACK); //x, y, radius, color
